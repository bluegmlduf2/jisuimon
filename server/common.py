...
def exception_handler(func):
    ...
    def inner_func():
        try:
            ...
            print("##### 접속자 IP주소 => ("+request.environ.get("HTTP_X_FORWARDED_FOR", request.remote_addr)+")#####\n")
...
def getMessage(code, param=None):
    MESSAGE = {
        ...
        702: "ファイル名が存在しません。<br>ファイルアップロード失敗しました",
        703: "空きファイルです。<br>ファイルアップロード失敗しました",
        704: "既に登録されているユーザーです",
        705: "",
        706: "ユーザー削除処理失敗しました",
        707: f"他のユーザーから登録された{param}が存在しますので削除できませんでした",
        708: "ファイルのサイズを確認してください。<br>ファイルのサイズが5MB以上の場合は登録できません。",
        # 실패 (서버)
...
# 이미지파일(섬네일사이즈)을 base64형식변환 (이미지파일,파일사이즈)
def imageParser(src,size):
    # 이미지파일->이미지파일사이즈변경->버퍼메모리저장->base64인코딩
    image = Image.open(src)
    buffered = BytesIO()  # 버퍼(임시메모리) 객체생성
    # 이미지파일을 리사이즈 후 임시 메모리 객체 버퍼에 저장
    image.resize((size, size)).save(buffered, format="JPEG")
