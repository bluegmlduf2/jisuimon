...
@comment_controller.route('/comment', methods=['POST', 'PATCH', 'DELETE'])
@check_token
@exception_handler
def comment():
    '''댓글 등록'''
    if request.method == 'POST':
        ...
        filteredArgs['commentContent'] = xssFilter(
            args['commentContent'])  # 댓글내용
    ...
    if request.method == 'PATCH':
        ...
        '''댓글 수정'''
        commentModel.updateComment(filteredArgs)
...
@comment_controller.route('/commentReply', methods=['POST', 'PATCH', 'DELETE'])
@check_token
@exception_handler
def commentReply():
    '''대댓글 등록'''
    if request.method == 'POST':
        ...
        # xssFilter처리된 값
        filteredArgs = copy.deepcopy(args)
        filteredArgs['commentReplyContent'] = xssFilter(
            args['commentReplyContent'])  # 댓글내용
    ...
    if request.method == 'PATCH':
        ...
        ### 유효성검사 ###
        # 공백 및 비어있는지 체크
        if not filteredArgs['commentReplyId']:
            raise UserError(701, 'コメントーID')
        if not filteredArgs['commentReplyContent']:
            raise UserError(701, 'コメント')
        ...
        '''대댓글 수정'''
        commentModel.updateCommentReply(filteredArgs)
